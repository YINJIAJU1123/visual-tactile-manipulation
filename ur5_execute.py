...
class MoveItIkDemo:
    def __init__(self, x1,y1,z1, orientation1, x2, y2, z2, orientation2):
        ...
        self.gs1_z_value = 0.0
        self.gs2_z_value = 0.0
        rospy.Subscriber("/gs1_z", Float64, self.gs1_callback)
        rospy.Subscriber("/gs2_z", Float64, self.gs2_callback)
        
        self.target_pose1, orientation1 = self.decide_optimal_rotation(x1,y1,z1, orientation1, x2, y2, z2, orientation2)
        print(self.target_pose1)
        
        new_position = self.adjust_position(self.target_pose1, move_distance = 0.1)
        print(new_position)
    ...
    def decide_optimal_rotation(self, x1,y1,z1, orientation1, x2, y2, z2, orientation2):
        ...
        if value1 < 10 and value11 < 10:
            rospy.loginfo("Both value1 and value11 are less than 10, stopping the system.")
            rospy.signal_shutdown("Values too low, stopping the system.")
            return None, None
        elif value1 > value11:
    ...
    def arm_move(self, target_pose1):
        ...
        attempts = 0     
        self.arm.set_start_state_to_current_state()
        while fraction < 0.75 and attempts < maxtries:
            (plan, fraction) = self.arm.compute_cartesian_path (
                                    waypoints,  
                                    0.02,        
                                    0.0,        
